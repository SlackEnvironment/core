...
const slackjsv1 = require('slackjsv1')
...
class Crypto {
  ...
  getBytes (transaction, skipSignature, skipSecondSignature) {
    if (!transaction.version || transaction.version === 1) {
      return slackjsv1.crypto.getBytes(transaction, skipSignature, skipSecondSignature)
    }
  }
  ...
  getId (transaction) {
    if (!transaction.version || transaction.version === 1) {
      return slackjsv1.crypto.getId(transaction)
    }
  ...
  getHash (transaction, skipSignature, skipSecondSignature) {
    if (!transaction.version || transaction.version === 1) {
      return slackjsv1.crypto.getHash(transaction, skipSignature, skipSecondSignature)
    }
  ...
  sign (transaction, keys) {
    if (!transaction.version || transaction.version === 1) {
      return slackjsv1.crypto.sign(transaction, keys)
    }
  ...
  secondSign (transaction, keys) {
    if (!transaction.version || transaction.version === 1) {
      return slackjsv1.crypto.secondSign(transaction, keys)
    }
  ...
  verify (transaction, network) {
    if (!transaction.version || transaction.version === 1) {
      return slackjsv1.crypto.verify(transaction, network)
    }
  ...
  verifySecondSignature (transaction, publicKey, network) {
    if (!transaction.version || transaction.version === 1) {
      try {
        return slackjsv1.crypto.verifySecondSignature(transaction, publicKey, network)
      } catch (error) {
        return false
      }
    }
