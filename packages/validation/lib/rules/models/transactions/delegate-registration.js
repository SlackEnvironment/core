...
module.exports = (transaction) => {
  const { error, value } = engine.validate(transaction, engine.joi.object({
    ...
    senderId: engine.joi.slackAddress(),
    recipientId: engine.joi.empty(),
    senderPublicKey: engine.joi.slackPublicKey().required(),
    ...
    asset: engine.joi.object({
      delegate: engine.joi.object({
        username: engine.joi.slackUsername().required(),
        publicKey: engine.joi.slackPublicKey()
      }).required()
    }).required(),
