...
const { configManager } = require('@slackenvironment/crypto')
...
class ConfigLoader {
  ...
  async setUp (options) {
    ...
    this.network = JSON.parse(process.env.SLACK_NETWORK)
  ...
  async copyFiles (dest) {
    if (!dest) {
      dest = `${process.env.SLACK_PATH_DATA}/config`
    }

    await fs.ensureDir(dest)

    return fs.copy(process.env.SLACK_PATH_CONFIG, dest)
  }
   ...
   */
  __getFiles () {
    const basePath = path.resolve(process.env.SLACK_PATH_CONFIG)
