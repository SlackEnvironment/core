...
// Examples! Cause you dum dums can't figure out this dumber code
  let client
  ...
  const sender = process.env.SLACK_CLIENT_EXAMPLE_SENDER
  if (!sender) {
    fail('It is necessary to establish the value of the environment variable "SLACK_CLIENT_EXAMPLE_SENDER" to an address')
  }

  const passphrase = process.env.SLACK_CLIENT_EXAMPLE_PASS
  if (!sender) {
    fail('It is necessary to establish the value of the environment variable "SLACK_CLIENT_EXAMPLE_PASS" to the passphrase of the "SLACK_CLIENT_EXAMPLE_SENDER" address')
  }
  ...
  const explorerUrl = 'https://dexplorer.slack.io/transaction/'
