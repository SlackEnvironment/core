...
const { ConnectionInterface } = require('@slackenvironment/core-database')
...
const { Block, Transaction } = require('@slackenvironment/crypto').models
...
module.exports = class SequelizeConnection extends ConnectionInterface {
  ...
  async make () {
    ...
    this.connection = new Sequelize({
      ...
      ...{
        operatorsAliases: Op,
        logging: process.env.NODE_ENV === 'test' && !process.env.SLACK_CI_TEST
      }
    })
  ...
  async buildWallets (height) {
    ...
    const spvPath = `${process.env.SLACK_PATH_DATA}/spv.json`
    if (fs.existsSync(spvPath)) {
      fs.removeSync(spvPath)
      logger.info('SLACK Core ended unexpectedly - resuming from where we left off :runner:')
