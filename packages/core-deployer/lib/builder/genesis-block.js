...
module.exports = class GenesisBlockBuilder {
  ...
  constructor (network, options) {
    ...
    slack.crypto.setNetworkVersion(this.prefixHash)
  }
  ...
  __createWallet () {
    ...
    const keys = slack.crypto.getKeys(passphrase, this.network)

    return {
      address: slack.crypto.getAddress(keys.publicKey, this.prefixHash),
      passphrase,
      keys
    }
  }
  ...
  __createTransferTransaction (senderWallet, receiverWallet, amount) {
    return this.__formatGenesisTransaction(
      slack.transaction.createTransaction(receiverWallet.address, amount, null, senderWallet.passphrase, undefined, this.prefixHash),
      senderWallet
    )
  }
  ...
  __createDelegateTransaction (wallet) {
    return this.__formatGenesisTransaction(
      slack.delegate.createDelegate(wallet.passphrase, wallet.username),
      wallet
    )
  }
  ...
  __formatGenesisTransaction (transaction, wallet) {
    ...
    transaction.signature = slack.crypto.sign(transaction, wallet.keys)
  ...
  __createGenesisBlock (data) {
    ...
    transactions.forEach(transaction => {
      const bytes = slack.crypto.getBytes(transaction)
