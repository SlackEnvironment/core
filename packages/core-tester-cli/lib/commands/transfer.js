...
const primaryAddress = slack.crypto.getAddress(slack.crypto.getKeys(config.passphrase).publicKey)
...
module.exports = async (options, wallets, slackPerTransaction, skipTestingAgain) => {
  ...
  let transactionAmount = (slackPerTransaction || 2) * Math.pow(10, 8)

  if (!slackPerTransaction && options.amount) {
    transactionAmount = options.amount
  }

  for (const id in wallets) {
    const wallet = wallets[id]
    const transaction = slack.transaction.createTransaction(
