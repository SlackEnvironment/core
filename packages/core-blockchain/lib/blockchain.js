...
const { slots } = require('@slackenvironment/crypto')
...
module.exports = class Blockchain {
  ...
  constructor (networkStart) {
    ...
    if (stateMachine.state.networkStart) {
      logger.warn('SLACK Core is launched in Genesis Start mode. This is usually for starting the first node on the blockchain. Unless you know what you are doing, this is likely wrong. :warning:')
      logger.info('Starting SLACK Core for a new world, welcome aboard :rocket:')
    }
  ...
  async start (skipStartedCheck = false) {
    ...
    if (skipStartedCheck || process.env.SLACK_SKIP_BLOCKCHAIN_STARTED_CHECK) {
      return true
    }

    // TODO: this state needs to be set after the state.lastBlock is available if SLACK_ENV=test
