...
class Monitor {
  ...
  async updateNetworkStatus () {
    try {
      ...
      if (Object.keys(this.peers).length < config.peers.list.length - 1 && process.env.SLACK_ENV !== 'test') {
        config.peers.list
  ...
  async acceptNewPeer (peer) {
    if (this.guard.isSuspended(peer) || this.guard.isMyself(peer) || process.env.SLACK_ENV === 'test') {
      return
    }
